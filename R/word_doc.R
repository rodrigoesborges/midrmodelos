...
#'        The 'mdir-template.docx' template implements most of the standard requirement at the UHH biology
...
#' \dontrun{
#'  # put in YAML header:
#'  output: midrmodelos::word_doc
#' }
word_doc <- function(toc = FALSE, toc_depth = 4, number_sections = FALSE,
  highlight = "default", reference_docx = "midr-template", font = "Georgia",
  language = "pt", dpi = 144, pandoc_args = NULL, ...) {

  # Font setting
  if (!font %in% c("Lato", "Georgia", "other")) {
    stop('Set the font option to "Georgia", "Lato" or "other".')
  }
  if (font == "Georgia") filename = "midr-template-georgia.docx"
  if (font == "Lato") filename = "midr-template-lato.docx"
  ...
  if (reference_docx == "midr-template") {
